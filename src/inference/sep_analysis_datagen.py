...
def get_param_values(nsamples):
    ...
    sep_array = np.linspace(SEP_MIN, SEP_MAX, SEP_STEP)
    t0_array = ORB_TO + np.random.uniform(0.01, 0.99, size=nsamples)*ORB_SIZE*ORB_DT

    transient_params = [m1_array, m2_array, d_array,\
          spin1_array, spin2_array, gw_beta_array, gw_lambda_array,\
            amp_array, beta_array, inj_point_array, t0_array]
    
    for i, arr in enumerate(transient_params):
        arr = np.repeat(arr, len(sep_array))
        transient_params[i] = arr

    sep_array = np.tile(sep_array, nsamples)
    transient_params.append(sep_array)
    return transient_params
...
def main():
    ...
    m1_array, m2_array, d_array, spin1_array, spin2_array, gw_beta_array, gw_lambda_array,\
         amp_array, beta_array, inj_point_array, t0_array, sep_array = get_param_values(NSAMPLES)
    ...
    jobs = [(m1_array[i], m2_array[i], d_array[i], 
             ...
             sep_array[i], t0_array[i], PIPE) for i in range(NSAMPLES*SEP_STEP)]
