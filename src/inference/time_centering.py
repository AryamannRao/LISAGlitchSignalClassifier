...
def load_best_model(weights_path, model_params):
    model = CNN(width=model_params['width'], bn=model_params['bn'], drop=model_params['drop'])
...
def create_inference_set(model, transient_type, keys=['QT', 't_qscan', 'f_qscan', 'tcen']):
    ...
    simulation_path = TRANSIENT_DICT[transient_type][MASS_CATEGORY]
    ...
    save_path = SAVE_DIR / f'{TRANSIENT_NAMES[transient_type]}_timecen.h5'
    h5file = create_imageset(save_path, TIME_STEP, RESOLUTION, keys=keys)
...
def run_inference(model):
    inference_gw_path = SAVE_DIR / 'gw_timecen.h5'
    inference_glitch_path = SAVE_DIR / 'glitch_timecen.h5'
    
    with h5py.File(inference_gw_path, 'r') as f:
        gws = f['QT'][:]
    with h5py.File(inference_glitch_path, 'r') as f:
        glitches = f['QT'][:]
        tcen_vals = f['tcen'][:]
...
def main():
    ...
    gw_inference_path = SAVE_DIR / f'{TRANSIENT_NAMES[1]}_timecen.h5'
    glitch_inference_path = SAVE_DIR / f'{TRANSIENT_NAMES[2]}_timecen.h5'

    if not os.path.exists(gw_inference_path):
        print("Creating GW inference dataset...")
        create_inference_set(model, transient_type=1)
    
    if not os.path.exists(glitch_inference_path):
