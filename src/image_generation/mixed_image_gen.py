...
def run_one_sample(args):
    # Convert one mixed TDI sample into Q-scans at valid randomized time centres.
    sep, X, Y, Z = args
    tdi_dict = make_tdi_dict(X, Y, Z, whiten=False)
    
    # Keep scan centres within the interval allowed by the transient separation.
    tcen_arr = np.random.uniform(-(3-np.abs(sep)), 3-np.abs(sep), size=TIME_STEP)*3600
...
def main():
    ...
    # Prepare job list
    jobs = [(sep_array[i], X_array[i], Y_array[i], Z_array[i])
